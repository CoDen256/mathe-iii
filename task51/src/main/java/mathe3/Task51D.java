...
public class Task51D {
    ...
    public static void main(String[] args) {
        List<Point> approxEuler = new EulerMethod().
                calculatePoints(Task51D::yDerivative, START_CONDITION, FROM, TO, H);
        new Plotter(approxEuler, 500000, 0.5)
                .function(Task51D::y).save(3);
    }
    ...
    public static double y(double x){
        return 1. / 5 * (Math.cos(2 * x) - 2 * Math.sin(2 * x));
    }
}
